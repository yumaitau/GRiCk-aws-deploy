...
Public buyer artifacts for [GRiCk on AWS Marketplace](https://aws.amazon.com/marketplace). Subscribe in the AWS Marketplace console **before** you pull the image or apply Terraform. The publisher does not host your data.

This repository is Terraform, Helm, and IAM only. The application image lives in AWS Marketplace ECR:
...
2. Set `marketplace_product_code` and `marketplace_product_sku` from the listing (Seller Central **product code** and **product ID**). Empty values disable the license check and Marketplace tasks will fail checkout.
...
Open that URL. First user registers at `/sign-up`, then creates the organisation at `/onboarding`. No seed admin is baked into the image.
...
HTTPS later: ACM certificate in the ALB Region, then set `certificate_arn` and `app_url = "https://your-hostname"`. TLS terminates on the ALB. It is not an extra IAM role.

Outbound email later: verify an Amazon SES identity, then set `ses_from_email`.
